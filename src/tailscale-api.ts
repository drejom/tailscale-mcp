...
export class TailscaleAPI {
   ...
   */
  private handleResponse<T>(response: AxiosResponse<T>): TailscaleAPIResponse<T> {
    return {
      success: true,
      data: response.data,
   ...
   */
  async getDevice(deviceId: string): Promise<TailscaleAPIResponse<TailscaleDevice>> {
    try {
   ...
   */
  async deauthorizeDevice(deviceId: string): Promise<TailscaleAPIResponse<void>> {
    try {
      const response = await this.client.post(`/device/${deviceId}/authorized`, {
        authorized: false
      });
   ...
   */
  async enableDeviceRoutes(deviceId: string, routes: string[]): Promise<TailscaleAPIResponse<void>> {
    try {
   ...
   */
  async disableDeviceRoutes(deviceId: string, routes: string[]): Promise<TailscaleAPIResponse<void>> {
    try {
   ...
   */
  async getFileSharingStatus(): Promise<TailscaleAPIResponse<{ fileSharing: boolean }>> {
    try {
   ...
   */
  async setFileSharingStatus(enabled: boolean): Promise<TailscaleAPIResponse<void>> {
    try {
   ...
   */
  async setDeviceExitNode(deviceId: string, routes: string[]): Promise<TailscaleAPIResponse<void>> {
    try {
      const response = await this.client.post(`/device/${deviceId}/routes`, {
        routes: routes
      });
   ...
   * Test ACL access
   */
  async testACLAccess(src: string, dst: string, proto?: string): Promise<TailscaleAPIResponse<any>> {
    try {
