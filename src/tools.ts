...
**Advanced Details:**
    ...
    try {
      ...
      switch (validatedArgs.operation) {
        case 'get_network_overview': {
          ...
          const exitNodes = devices.filter((d: any) => 
            d.advertisedRoutes?.includes('0.0.0.0/0') || d.advertisedRoutes?.includes('::/0')
          );
  ...
  - Non-Compliant: ${nonCompliantDevices.length}`;
          ...
          if (nonCompliantDevices.length > 0) {
            ...
            resultText += nonCompliantDevices.map(device => 
              `  - ${device.name}: ${device.issues.join(', ')}`
            ).join('\n');
          }
